<template>
  <div class="tinymce-editor">
    ...
    <div :style="slotStyle" @click="getContentFun">
      <slot></slot>
    </div>
  </div>
</template>
...
export default defineComponent({
  props: {
    ...
    plugins: {
      type: [String, Array],
      default:
        'lists fullscreen emoticons autosave image table wordcount emoticons',
    },
    //工具
    toolbar: {
      type: [String, Array],
      ...
      default:
        'bold italic underline strikethrough restoredraft image fullscreen emoticons | fontsizeselect | forecolor backcolor | alignleft aligncenter alignright alignjustify|bullist numlist |outdent indent blockquote | undo redo | removeformat',
    },
    url: {
      required: true,
      type: String,
    ...
    function init() {
      window.tinymce.init({
        ...
        content_style:
          ...
          '*::-webkit-scrollbar-thumb{border-radius: 3px;background: #bac3d9;}',
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          ...
          // 改为自己接口路径
          axios.post(props.url, formdata).then(res => {
            // 我的接口返回数据格式
